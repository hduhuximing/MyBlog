...
public class RemoverServiceImpl implements RemoverService {
    ...
    public void run(Long typeId, @Validated HunterConfig config, PrintWriter writer) {
        ...
        if (null == typeId) {
            writerUtil.print("校验不通过！请选择文章分类......",
                    String.format("共耗时 %s ms.",
        ...
        if (CollectionUtils.isEmpty(list)) {
            writerUtil.print(String.format("未抓取到任何内容，请确保连接[<a href=\"%s\" target=\"_blank\">%s</a>]" +
                            "是否正确并能正常访问！共耗时 %s ms.",
                    config.getEntryUrls().get(0),
                    config.getEntryUrls().get(0),
                    (System.currentTimeMillis() - start))).shutdown();
            return;
        }
